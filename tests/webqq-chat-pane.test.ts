...
describe('WebQQ 聊天区域', () => {
  ...
  it('组合顶栏、消息列表与发送控件', () => {
    ...
    expect(pageSource).toContain(':reveal-request="messageRevealRequest"')
    expect(pageSource).toContain('@open-message="navigateToHistoryMessage"')
    expect(pageSource).toContain('createMessageNavigationShell(workspaceController, activeSpaceId, enterTestSpace)')
    const navigationSource = readFileSync(resolve('client/webqq/message-navigation.ts'), 'utf8')
    expect(navigationSource).toContain('await enterSpace(spaceId)')
    expect(navigationSource).toContain('await controller.selectOperator(target.botId)')
    expect(navigationSource).toContain('controller.currentOperatorId.value !== target.botId')
    expect(navigationSource).toContain('controller.selectConversation(target.conversationId)')
  ...
  it('在顶栏提供查找聊天记录并定位历史消息', () => {
    ...
    expect(source).toContain('revealRequest?: WebqqMessageRevealRequest')
    expect(source).toContain('watch(() => props.revealRequest?.seq')
    expect(source).toContain('async function revealRequestedMessage()')
    expect(source).toContain("request.conversationId !== props.model.conversationId")
