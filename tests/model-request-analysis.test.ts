...
describe('模型请求分析展示模型', () => {
  ...
  it('history_new 和 history_last 默认渲染消息预览，并保留原始 XML 切换', () => {
    ...
    expect(view).toContain('v-else-if="historyPreview(variable) && !rawHistoryVariables.has(variable.id)"')
    ...
    expect(view).toContain('@click="toggleHistoryRaw(variable.id)"')
    expect(preview).toContain('v-for="item in metadata(message)"')
    expect(preview).toContain(':class="{ \'is-bot\': isBotMessage(message) }"')
    ...
    expect(preview).toContain('<MessageJumpButton v-if="isBotMessage(message) && canOpenMessage(message)" :message-id="message.messageId!" placement="before"')
    expect(preview).toContain('<MessageJumpButton v-if="!isBotMessage(message) && canOpenMessage(message)" :message-id="message.messageId!" placement="after"')
    expect(preview).not.toContain('webqq-model-history-head')
    expect(preview).toContain("emit('openMessage', buttonProps.messageId)")
    expect(preview).not.toContain(':title=')
    expect(preview).toContain('variant="secondary"')
    expect(preview).toContain(':class="metadataClass(message, item)"')
    expect(preview).toContain('message.id === props.botId')
    expect(preview).toContain('return isBotMessage(message) ? items.reverse() : items')
    expect(preview).toContain("isBotMessage(message) ? 'is-name is-bot' : 'is-name is-user'")
    ...
    expect(styles).toContain('.webqq-model-history-meta .sandbox-badge.is-name.is-user')
    ...
    expect(styles).toMatch(/\.webqq-model-history-content \{[^}]*width: fit-content;[^}]*max-width: 100%;/s)
    expect(styles).toMatch(/\.webqq-model-history-jump \{[^}]*color: var\(--webqq-muted\);[^}]*opacity: 0;/s)
    expect(styles).toMatch(/\.webqq-model-history-jump\.is-before,\n\.webqq-model-history-jump\.is-after \{[^}]*width: 1em;[^}]*height: 1em;[^}]*vertical-align: -0\.1em;/s)
    expect(styles).toContain('.webqq-model-history-jump.is-before { margin-inline-end: 5px; }')
    expect(styles).toContain('.webqq-model-history-jump.is-after { margin-inline-start: 5px; }')
    expect(styles).toContain('.webqq-model-history-message:hover .webqq-model-history-jump')
    expect(styles).toContain('.webqq-model-history-message:focus-within .webqq-model-history-jump')
    ...
    expect(view).toContain("openMessage: [target: WebqqMessageNavigationTarget]")
    expect(trajectory.match(/@open-message="emit\('open-message', \$event\)"/g)).toHaveLength(2)
    expect(workspace.match(/@open-message="emit\('openMessage', \$event\)"/g)).toHaveLength(2)
  })
