...
describe('模型请求分析展示模型', () => {
  ...
  it('history_new 和 history_last 默认渲染消息预览，并保留原始 XML 切换', () => {
    ...
    expect(preview).toContain('v-for="(line, lineIndex) in contentLines(message)"')
    expect(preview).toContain("'is-single-visual-line': singleVisualLines.has(contentLineKey(message, index, lineIndex))")
    expect(preview).toContain(':data-history-content-line="contentLineKey(message, index, lineIndex)"')
    ...
    expect(preview).toContain("'--webqq-model-history-content-max-width'")
    ...
    expect(styles).toMatch(/\.webqq-model-history-content \{[^}]*width: fit-content;[^}]*max-width: min\(100%, var\(--webqq-model-history-content-max-width, 100%\)\);/s)
    expect(styles).toMatch(/\.webqq-model-history-content-line \{[^}]*overflow-wrap: anywhere;[^}]*white-space: pre-wrap;/s)
    expect(styles).toMatch(/\.webqq-model-history-message\.is-bot \.webqq-model-history-content-line \{[^}]*align-self: flex-end;[^}]*text-align: left;/s)
    expect(styles).toMatch(/\.webqq-model-history-message\.is-bot \.webqq-model-history-content-line\.is-single-visual-line \{[^}]*text-align: right;/s)
