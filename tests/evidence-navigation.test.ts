...
): Pick<SandboxModelRequestTrajectory, 'mode' | 'records'> {
  return {
    mode,
...
describe('证据导航', () => {
  describe('进入模型请求视图', () => {
    it('从聊天消息进入时把主环境引用映射为主环境意图，不携带证据定位段', () => {
      const navigation = createEvidenceNavigation()

      expect(navigation.enterFromMessage({ scopeId: 'main', recordId: 'request:main' })).toEqual({
        seq: 1,
        scope: { scope: 'main' },
    ...
    it('从聊天消息进入时把测试空间引用映射为空间意图', () => {
      const navigation = createEvidenceNavigation()

      expect(navigation.enterFromMessage({ scopeId: 'space-a', recordId: 'request:space' })).toEqual({
        seq: 1,
        scope: { scope: 'space', spaceId: 'space-a' },
        recordId: 'request:space',
      })
      expect(navigation.entryState.value?.spaceId).toBe('space-a')
    })
    ...
    it('从预设工作台进入时携带证据定位段并给出正在定位的进行态文案', () => {
      ...
      expect(navigation.entryState.value?.status).toBe('正在打开匹配的模型请求并定位精确文本…')
    })
    ...
    it('两条入口路径交出的进入状态在保护标记上完全一致', () => {
      const fromMessage = createEvidenceNavigation()
      fromMessage.enterFromMessage({ scopeId: 'main', recordId: 'record-1' })
      const fromPreset = createEvidenceNavigation()
      fromPreset.enterFromPreset(matched())

      const projection = (state: typeof fromMessage.entryState.value) => ({
      ...
      expect(projection(fromPreset.entryState.value)).toEqual(projection(fromMessage.entryState.value))
      expect(fromPreset.entryState.value?.navigationSelection).toBe(true)
      expect(fromMessage.preserveSelectionOnFilterChange()).toBe(true)
      expect(fromPreset.preserveSelectionOnFilterChange()).toBe(true)
    })
    ...
    it('一次性令牌与进入状态绑定，消费一次后失效', () => {
      ...
      navigation.enterFromMessage({ scopeId: 'main', recordId: 'record-1' })
    ...
    it('进入状态没有改变筛选时令牌立即失效', () => {
      const navigation = createEvidenceNavigation()
      const intent = navigation.enterFromMessage({ scopeId: 'main', recordId: 'record-1' })

      navigation.applyEntry(intent.seq, false)

      expect(navigation.preserveSelectionOnFilterChange()).toBe(false)
    })

    it('进入状态改变了筛选时令牌保留到侦听器消费', () => {
      const navigation = createEvidenceNavigation()
      const intent = navigation.enterFromMessage({ scopeId: 'main', recordId: 'record-1' })
    ...
    it('过期编号不消费进入状态', () => {
      ...
      navigation.applyEntry(99, true)
      expect(navigation.entryState.value?.seq).toBe(1)
    })
  ...
  describe('预设工作台原点', () => {
    ...
    it('从聊天消息进入没有回程，并清掉上一次的预设原点', () => {
      const navigation = createEvidenceNavigation()
      navigation.enterFromPreset(matched())
      expect(navigation.returnTarget.value).toBeDefined()

      navigation.enterFromMessage({ scopeId: 'main', recordId: 'record-2' })
  ...
  describe('module 归属', () => {
    it('四个旧 module 与其测试文件已删除，无残留引用', () => {
      ...
      for (const name of removed) {
        expect(existsSync(resolve('client/webqq', `${name}.ts`))).toBe(false)
