...
describe('WebQQ 模型请求工作台', () => {
  it('从最左侧导航进入独立视图，并提供分类、分页摘要、详情和未归属二次确认', () => {
    ...
    expect(workspaceSource).toMatch(/function refresh\([\s\S]*emit\('trajectory', \{ \.\.\.scope, recordId: selectedRecordId\.value, mode: trajectoryMode\.value \}\)/)
    ...
    expect(workspaceSource).toContain("trajectoryMode = ref<'request' | 'conversation'>('request')")
    expect(trajectorySource).toContain('单请求')
    expect(trajectorySource).toContain('完整会话')
    expect(trajectorySource).toContain('上下文')
    ...
    expect(styles).toMatch(/\.webqq-model-trajectory-span\.is-request\s*\{[^}]*top:\s*19px/s)
    expect(styles).toMatch(/\.webqq-model-trajectory-span\.is-context\s*\{[^}]*top:\s*5px/s)
    expect(styles).toMatch(/\.webqq-model-trajectory-span\.is-tools\s*\{[^}]*top:\s*33px/s)
