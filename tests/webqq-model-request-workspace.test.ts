...
describe('WebQQ 模型请求工作台', () => {
  it('页面入口与装配：最左侧导航进入独立视图，主页面传机器人目录、访问计数与容量水位', () => {
    ...
    expect(pageSource).toContain(':capacity="modelRequestWorkspaceModel.capacity"')
  ...
  it('详情概览与元信息：详情头部的导航与视图切换、概览格、用量格、元信息列表与请求头树', () => {
    ...
    expect(styles).toMatch(/\.webqq-model-request-actions\s*\{[^}]*flex-direction:\s*column[^}]*align-items:\s*flex-end/s)
    ...
    expect(workspaceSource).toMatch(/webqq-model-request-overview[\s\S]*statusLabel\(detail\.status\)[\s\S]*来源[\s\S]*模型 ID[\s\S]*耗时[\s\S]*字段[\s\S]*消息[\s\S]*工具/)
    ...
    expect(workspaceSource).toMatch(/webqq-model-request-entities[\s\S]*v-for="chip in entityChips"[\s\S]*<strong>{{ chip\.label }}<\/strong>[\s\S]*webqq-model-request-entity-value[\s\S]*{{ chip\.value }}/)
    ...
    expect(styles).toMatch(/\.webqq-model-request-entities\s*\{[^}]*flex-wrap:\s*wrap[^}]*font-family:\s*var\(--webqq-font-mono\)/s)
    expect(styles).toMatch(/\.webqq-model-request-entity strong\s*\{[^}]*color:\s*var\(--webqq-accent\)/s)
  ...
  it('错误诊断：错误码、报错、原始原因与可能的原因四项及其容器', () => {
    ...
    expect(workspaceSource).toContain('<dt class="webqq-model-request-error-code-label">错误码</dt>')
    ...
    expect(workspaceSource).toContain('webqq-model-request-error-code')
    expect(workspaceSource).toContain('webqq-model-request-error-trace')
    expect(styles).toContain('.webqq-model-request-error-diagnostic')
    expect(styles).toContain('.webqq-model-request-error-causes')
    expect(styles).toContain('.webqq-model-request-error-code')
    expect(styles).toContain('.webqq-model-request-error-trace')
  })
  ...
  it('请求体与响应视图：三个页签的顺序、响应采集状态文案、内容预览分区与图片预览开关', () => {
    ...
    expect(responsePreviewSource.match(/class="webqq-model-response-tool-payload"/g)).toHaveLength(2)
    ...
    const payloadRule = styles.slice(styles.indexOf('.webqq-model-response-tool-payload {')).split('}')[0]
    ...
    expect(responsePreviewSource.match(/class="webqq-model-response-tool-header"/g)).toHaveLength(2)
    expect(styles).toContain('.webqq-model-response-tool-header {')
    expect(styles).not.toContain('.webqq-model-response-tool > div')
    ...
    expect(styles).toMatch(/\.webqq-model-response-section\.is-content,\s*\n\s*\.webqq-model-response-section\.is-reasoning\s*\{[^}]*border-left-color:\s*var\(--webqq-role-response\)/s)
  ...
  it('请求组成图：轨道缩放控件、变量分段标签与空态文案', () => {
    ...
    expect(trajectorySource).toMatch(/webqq-model-trajectory-controls[\s\S]*webqq-model-trajectory-control-query[\s\S]*webqq-model-trajectory-composition-zoom[\s\S]*webqq-model-trajectory-search[\s\S]*webqq-model-trajectory-composition-shell/)
    ...
    expect(styles).toMatch(/\.webqq-model-trajectory-control-query\s*\{[^}]*display:\s*flex[^}]*gap:\s*8px[^}]*margin-left:\s*auto/s)
    expect(styles).toMatch(/\.webqq-model-trajectory-composition-zoom\s*\{[^}]*position:\s*relative[^}]*flex:\s*0 0 auto[^}]*gap:\s*1px/s)
    expect(styles).toMatch(/\.webqq-model-trajectory-search\s*\{[^}]*flex:\s*0 1 164px/s)
    // 搜索框宽度必须是确定长度：父级是收缩到内容的 flex 盒，百分比宽度在 Firefox 下会让父级
    // max-content 回退到 input 固有宽度，多出的宽度会全部留在搜索框右侧，破坏工具栏两侧对称。
    expect(styles).toMatch(/\.webqq-model-trajectory-search\s*\{[^}]*width:\s*164px/s)
    expect(styles).not.toMatch(/\.webqq-model-trajectory-search\s*\{[^}]*width:[^;]*%/s)
    // 左侧 ghost 按钮自带 6px 内边距，搜索框带边框，补齐同样的 6px 两侧可视留白才对称。
    expect(styles).toMatch(/\.webqq-model-trajectory-search\s*\{[^}]*margin-right:\s*6px/s)
    expect(styles).not.toContain('.webqq-model-trajectory-composition-zoom.is-expanded')
    expect(styles).not.toMatch(/\.webqq-model-trajectory-composition-zoom\s*\{[^}]*border:/s)
    expect(styles).not.toMatch(/\.webqq-model-trajectory-composition-zoom\s*\{[^}]*background:/s)
    ...
    expect(styles).toMatch(/\.webqq-model-trajectory-composition-bar\.is-variable\s*\{[^}]*background-color:\s*var\(--webqq-role-variable\)/s)
    ...
    for (const modifier of ['is-user', 'is-variable', 'is-assistant', 'is-tool-definition', 'is-tool-interaction']) {
      expect(styles).not.toMatch(new RegExp(`\\.webqq-model-trajectory-composition-bar\\.${modifier}\\s*\\{[^}]*background:`, 's'))
    }
  ...
  it('轨迹账本与检查器：模式切换、账本列与种类标签、请求折叠、检查器容器与工具栏折叠', () => {
    ...
    expect(styles).not.toMatch(/\.webqq-model-trajectory-control-filters[^}]*cursor:/s)
  })
  ...
  it('样式：工作台栅格、粘性轨迹头、徽标与角色配色', () => {
    ...
    expect(styles).toMatch(/\.webqq-model-request-detail\.has-sticky-trajectory \{[^}]*padding-top: 0;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.has-sticky-trajectory > header:first-child \{[^}]*padding-top: 16px;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.is-trajectory-view \{[^}]*overflow: hidden;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.is-trajectory-view > \.webqq-model-trajectory \{[^}]*position: relative;[^}]*height: 100%[^}]*min-height: 0;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.is-trajectory-view \.webqq-model-trajectory-header \{[^}]*position: absolute;[^}]*inset: 0 0 auto;[^}]*overflow: clip;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.is-trajectory-view \.webqq-model-trajectory-header::before \{[^}]*z-index: 0;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.is-trajectory-view \.webqq-model-trajectory-header > \* \{[^}]*z-index: 1;/s)
    expect(styles).toMatch(/\.webqq-model-request-detail\.is-trajectory-view \.webqq-model-trajectory-table::before \{[^}]*height: var\(--webqq-model-trajectory-sticky-height[^}]*content: "";/s)
    expect(styles).toMatch(/\.webqq-workspace\.is-frosted \.webqq-model-trajectory \{[^}]*background: transparent;/s)
    expect(styles).not.toMatch(/\.webqq-model-request-detail\.is-trajectory-view \.webqq-model-trajectory-header \{[^}]*top:\s*-/s)
    ...
    expect(styles).toContain('--webqq-role-variable: #a13d76')
    expect(styles).toContain('--webqq-role-variable: #dd79b2')
  })
