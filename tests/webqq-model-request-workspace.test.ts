...
describe('WebQQ 模型请求工作台', () => {
  it('从最左侧导航进入独立视图，并提供分类、分页摘要、详情和未归属二次确认', () => {
    ...
    // 概览格三格读记录字段与共享模型证据投影派生的计数，缺省时显示占位符而不是零。
    expect(workspaceSource).not.toContain('summary.keys')
    expect(workspaceSource).toContain("detail.requestBodyKeyCount ?? '—'")
    expect(workspaceSource).toContain("detail.evidenceCounts?.requestMessageCount ?? '—'")
    expect(workspaceSource).toContain("detail.evidenceCounts?.toolDefinitionCount ?? '—'")
    ...
    expect(workspaceSource).toContain("bodyView = ref<'request' | 'response' | 'analysis'>('analysis')")
    ...
    expect(trajectorySource).toMatch(/<template v-if="compositionZoom > COMPOSITION_ZOOM_MIN">[\s\S]*?composition-zoom-value[\s\S]*?IconZoomOut[\s\S]*?<\/template>[\s\S]*?IconZoomIn/)
    expect(trajectorySource).toContain('COMPOSITION_ZOOM_MIN = 1')
    expect(trajectorySource).toContain('@wheel="handleCompositionWheel"')
    expect(trajectorySource).toContain('@pointerdown="handleCompositionPointerDown"')
    expect(trajectorySource).toContain('@pointermove="handleCompositionPointerMove"')
    expect(trajectorySource).toContain('viewport.setPointerCapture(event.pointerId)')
    expect(trajectorySource).toContain('viewport.scrollLeft = drag.scrollLeft - delta')
    expect(trajectorySource).toContain('suppressCompositionClickUntil = performance.now() + 250')
    expect(trajectorySource).toContain('if (!event.ctrlKey || event.deltaY === 0) return')
    expect(trajectorySource).toContain('event.preventDefault()')
    expect(trajectorySource).toContain('COMPOSITION_ZOOM_MIN = 1')
    ...
    expect(trajectorySource).toContain('props.navigation.locateEvidence(')
    ...
    expect(workspaceSource).toContain("detailView.value = 'evidence'")
    expect(workspaceSource).toContain("bodyView.value = 'analysis'")
    expect(workspaceSource).toMatch(/function openRelatedRequest\([\s\S]*bodyView\.value = 'request'/)
    expect(workspaceSource).toContain("responseView.value = 'content'")
    // 返回快照与「详情到达后才消费」的判定都在证据导航 module 里；工作台不再持有任何镜像状态。
    expect(workspaceSource).toContain('navigation.pushViewSnapshot({')
    expect(workspaceSource).toContain('navigation.beginViewReturn()')
    expect(workspaceSource).toContain('navigation.takeViewRestore(props.detail?.id)')
    expect(workspaceSource).toContain('const returnLabel = computed(() => {')
    ...
    // 进入状态、开始返回、目标详情到达后再次恢复：三处都按 module 交出的数据写页签。
    expect(workspaceSource.match(/detailView\.value = state\.detailView/g)).toHaveLength(3)
    ...
    expect(trajectorySource).toContain('ledgerScrollRestore.restore(state.trajectory.scrollTop)')
    ...
    expect(trajectorySource).toContain('selectedRowId.value = state.trajectory.rowId')
    expect(workspaceSource).toContain("mode: 'request'")
    ...
    expect(trajectorySource).toContain('v-for="row in orderedLedgerRows"')
    expect(trajectorySource).toContain("trajectorySortOrder = ref<ModelRequestTrajectorySortOrder>('desc')")
    expect(trajectorySource).toContain("mode === 'conversation' && !analysis")
    ...
    expect(trajectorySource).toContain('orderModelRequestTrajectoryRows(ledgerRows.value, trajectorySortOrder.value)')
    expect(trajectorySource).toContain('collapsedRequestIds')
    expect(trajectorySource).toContain('toggleRequestCollapsed(row)')
