...
describe('WebQQ 模型请求工作台', () => {
  it('从最左侧导航进入独立视图，并提供分类、分页摘要、详情和未归属二次确认', () => {
    ...
    expect(workspaceSource).toContain('JSON.stringify(props.detail.headers, null, 2)')
    expect(workspaceSource).toMatch(/<pre v-if="headersExpanded" class="webqq-model-request-header-json">/)
    expect(styles).toContain('.webqq-model-request-header-toggle')
    expect(styles).toContain('.webqq-model-request-header-json')
