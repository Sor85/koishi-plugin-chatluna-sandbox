...
describe('WebQQ 模型请求工作台', () => {
  it('从最左侧导航进入独立视图，并提供分类、分页摘要、详情和未归属二次确认', () => {
    ...
    expect(trajectorySource).toMatch(/composition-zoom-value[\s\S]*IconZoomOut[\s\S]*IconZoomIn/)
    ...
    expect(styles).toMatch(/\.webqq-model-trajectory-composition-zoom\s*\{[^}]*position:\s*absolute[^}]*top:\s*2px[^}]*right:\s*4px/s)
