...
describe('WebQQ 模型请求工作台', () => {
  it('从最左侧导航进入独立视图，并提供分类、分页摘要、详情和未归属二次确认', () => {
    ...
    expect(styles).toMatch(/\.webqq-model-trajectory-composition-zoom\s*\{[^}]*position:\s*absolute[^}]*top:\s*2px[^}]*right:\s*4px[^}]*isolation:\s*isolate/s)
    expect(styles).toMatch(/\.webqq-workspace\.is-frosted \.webqq-model-trajectory-composition-zoom::before\s*\{[^}]*inset:\s*0[^}]*backdrop-filter:\s*saturate\(180%\) blur\(14px\)/s)
