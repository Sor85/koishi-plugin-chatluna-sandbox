...
describe('WebQQ 模型请求工作台', () => {
  it('从最左侧导航进入独立视图，并提供分类、分页摘要、详情和未归属二次确认', () => {
    ...
    expect(workspaceSource).toContain('CHATLUNA_ERROR_CODE_DOCUMENTATION_URL')
    expect(workspaceSource).toContain('getChatLunaErrorPossibleCauses')
    ...
    expect(detailHeader).not.toContain('返回轨迹')
    ...
    expect(pageHeader).toContain('IconArrowLeft')
    expect(pageHeader).toContain('返回轨迹')
    expect(pageHeader.indexOf('刷新')).toBeLessThan(pageHeader.indexOf('返回轨迹'))
    ...
    expect(workspaceSource).toContain('detailScrollRestore.restore(state.detailScrollTop)')
    expect(trajectorySource).toContain('ledgerScrollRestore.restore(state.scrollTop)')
