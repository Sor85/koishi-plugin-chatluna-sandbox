...
describe('环境管理组件传输边界', () => {
  ...
  it('三类实体编辑与创建表单通过二级页面选择内置或本地头像', () => {
    ...
    expect(source).toContain(':show-bot-badge="false"')
    ...
    expect(create).toContain(':show-bot-badge="false"')
  ...
  it('群组编辑以全部用户和机器人头像管理成员资料', () => {
    ...
    expect(styles).toContain('--webqq-bot-badge-offset: -2px')
    expect(styles).toContain('.webqq-group-member-card.is-selected .webqq-group-member-avatar .webqq-avatar-bot-badge')
