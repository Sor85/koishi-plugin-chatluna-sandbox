...
describe('环境管理组件传输边界', () => {
  ...
  it('群组编辑以全部用户和机器人头像管理成员资料', () => {
    ...
    expect(source).toContain('class="chatluna-sandbox-group-members-editor"')
    expect(source).not.toContain('class="webqq-secondary-divider chatluna-sandbox-group-members-editor"')
  ...
  it('Portal 二级菜单提供完整的控件令牌，避免控件回退为黑色描边', () => {
    ...
    expect(primitives).toContain('background: color-mix(in srgb, var(--webqq-bg) 72%, transparent)')
    expect(primitives).toContain('backdrop-filter: saturate(180%) blur(20px)')
    expect(primitives).toContain('box-shadow: inset 0 0 0 2px var(--webqq-accent-shadow)')
