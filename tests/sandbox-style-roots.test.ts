...
import { SANDBOX_STYLE_ROOTS } from '../scripts/sandbox-style-roots.mjs'
...
const PORTAL_INVENTORY: Array<[string, string]> = [
  ...
  ['client/components/ui/select/SelectContent.vue', '.sandbox-select-content'],
  ['client/components/ui/tooltip/TooltipContent.vue', '[data-slot="tooltip-content"]'],
  ['client/components/ui/context-menu/ContextMenuContent.vue', '[data-slot="chatluna-sandbox-context-menu-content"]'],
  ['client/components/ui/context-menu/ContextMenuSubContent.vue', '[data-slot="chatluna-sandbox-context-menu-sub-content"]'],
]
...
describe('沙盒渲染根清单', () => {
  it('排版令牌与浮层基准都写在同一份渲染根清单上', () => {
    ...
    // 令牌规则：webqq-tokens.css 里第一条声明 --webqq-font-3xs 的规则。
    expect(selectorList(tokens, '--webqq-font-3xs')).toEqual(SANDBOX_STYLE_ROOTS)
    // 浮层基准规则：webqq-primitives.css 里声明 tab-size 的那条。
    expect(selectorList(primitives, 'tab-size: 4')).toEqual(SANDBOX_STYLE_ROOTS)
  })

  it('每个 teleport / Portal 浮层的渲染根都在清单里', () => {
    for (const [file, root] of PORTAL_INVENTORY) {
      expect(SANDBOX_STYLE_ROOTS, `${file} 的浮层根 ${root} 不在 SANDBOX_STYLE_ROOTS 里`).toContain(root)
  ...
  it('新增 teleport / Portal 时必须同步清单', () => {
    // 计数守卫：这两个数字变化说明有人加了浮层，必须同时补 PORTAL_INVENTORY 与 SANDBOX_STYLE_ROOTS，
