...
/**
 * 各能力的模板给 shadcn 控件加的本插件类名。状态类走 `:class` 绑定，本身就是复合选择器，不在此列。
 *
 * 不扫 `client/components/ui/`：那里的封装写在自己根元素上的类（`.sandbox-dialog-content` 这类）是
 * 浮层的样式根，令牌与排版基线按 `scripts/sandbox-style-roots.mjs` 的清单原样落在根类上，
 * 由 `tests/sandbox-style-roots.test.ts` 逐项守着，不能为了特异性改写成别的形态。
 */
function shadcnControlClasses(): string[] {
  const tags = shadcnControlTags()
  const classes = new Set<string>()
  for (const { path, source } of readClientTemplates()) {
    if (path.includes(join('client', 'components', 'ui'))) continue
    for (const match of source.matchAll(/<([A-Z]\w*)\b([^>]*)>/g)) {
      ...
      for (const token of attribute[1]!.split(/\s+/)) {
        if (/^(webqq|sandbox|chatluna-sandbox)-/.test(token)) classes.add(token)
      }
    }
  }
  return [...classes].sort()
