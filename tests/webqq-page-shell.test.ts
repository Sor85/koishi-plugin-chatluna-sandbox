...
describe('WebQQ 主页面装配', () => {
  it('只装配工作区区域与组合模块', () => {
    ...
    expect(shellSource).toContain('buildForwardPreviewMap')
    expect(shellSource).toContain('forwardPreviews:')
    expect(shellSource).toContain('sendForwardMessage')
    expect(shellSource).toContain('getForwardMessage')
    expect(shellSource).toContain('searchConversationMessages')
    expect(shellSource).toContain('forwardTargets:')
    expect(shellSource).toContain('forwardMediaIds')
