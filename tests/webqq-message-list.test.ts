...
describe('WebQQ 消息列表', () => {
  it('从页面提取消息渲染与消息交互', () => {
    ...
    expect(source).toContain('@wheel.passive="finishMessageListScrollRestore"')
    expect(source).toContain('@touchstart.passive="finishMessageListScrollRestore"')
    expect(source).toContain('@pointerdown="finishMessageListScrollRestore"')
    expect(source).toContain('if (!element || restoringScrollState) return')
