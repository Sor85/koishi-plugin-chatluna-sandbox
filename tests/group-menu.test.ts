...
describe('群成员右键菜单权限', () => {
  it('群主可以管理普通成员和管理员', () => {
    expect(getGroupMemberMenuActions(owner, member, false)).toEqual(['poke', 'set-card', 'kick', 'set-admin'])
    expect(getGroupMemberMenuActions(owner, admin, false)).toEqual(['poke', 'set-card', 'kick', 'unset-admin'])
  })
  ...
  it('机器人不能被设为管理员', () => {
    expect(getGroupMemberMenuActions(owner, member, true)).toEqual(['poke', 'set-card', 'kick'])
  })
})
