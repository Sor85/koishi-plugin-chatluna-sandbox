...
describe('群成员右键菜单权限', () => {
  it('群主可以管理普通成员和管理员', () => {
    expect(getGroupMemberMenuActions(owner, member)).toEqual(['mention', 'poke', 'set-card', 'set-admin', 'transfer-owner', 'kick'])
    expect(getGroupMemberMenuActions(owner, admin)).toEqual(['mention', 'poke', 'set-card', 'unset-admin', 'transfer-owner', 'kick'])
  })
