...
describe('证据阅读顺序', () => {
  it('声明的档位顺序把响应排在工具之前', () => {
    expect(SANDBOX_EVIDENCE_READING_ORDER).toEqual([
  ...
  it('响应侧证据整体归 Response 一档', () => {
    for (const kind of ['assistant', 'tool-call', 'tool-result'] as const) {
      expect(sandboxEvidenceReadingGroup({ kind, source: 'response' })).toBe('response')
    }
  })

  it('三种工具证据在请求侧收成 Tool 一档', () => {
    for (const kind of ['tool-definition', 'tool-call', 'tool-result'] as const) {
      expect(sandboxEvidenceReadingGroup({ kind, source: 'request' })).toBe('tool')
    }
  })

  it('工具调用跟着发起它的那条消息', () => {
    expect(sandboxEvidenceReadingGroup({ kind: 'tool-call', parent: 'assistant' })).toBe('assistant')
    // 响应侧优先：整张响应卡片是一档，父档不能把它拆回 assistant。
    expect(sandboxEvidenceReadingGroup({ kind: 'tool-call', source: 'response', parent: 'assistant' })).toBe('response')
  })

  it('未知档位排在末尾而不是把已知档位顶下去', () => {
    expect(sandboxEvidenceReadingRank('system')).toBe(0)
    expect(sandboxEvidenceReadingRank('tool')).toBe(SANDBOX_EVIDENCE_READING_ORDER.length - 1)
    expect(sandboxEvidenceReadingRank('未知' as SandboxEvidenceReadingGroup)).toBe(SANDBOX_EVIDENCE_READING_ORDER.length)
  })
  ...
  it('分析导航的分组顺序就是这一份声明的子序列', () => {
    ...
    const ranks = navigation.groups.map(({ key }) => sandboxEvidenceReadingRank(key))
  ...
  it('账本事件行与导航把同一条证据排在同一档', () => {
    const request = detail()
    const navigation = buildModelRequestAnalysisNavigation(parseModelRequestConversationDetail(request), request)
    const trajectory = buildSandboxModelRequestTrajectory({ record: request, mode: 'request' })
    ...
    const ranks = trajectory.rows.flatMap((row) => {
      ...
      return [sandboxEvidenceReadingRank(group!)]
    })
