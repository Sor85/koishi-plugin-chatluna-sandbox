...
- [x] Chrome 与 Firefox 均验证请求、响应、工具事件、搜索、定位和 pending 自动刷新
...
- 浏览器验证已完成：在完整 Koishi 开发环境（portal 实时链路，控制台 `http://127.0.0.1:5140`）
  中用真实 Gemini `generateContent` 记录验证，Chromium 与 Firefox 各 18/18 项通过，全程无控制台错误。
  覆盖：请求体 JSON 树与概览、响应格式化内容与原文视图、TOOL DEFS / TOOL CALL / TOOL RESULT 分组、
  分析导航 45 项抽样 15 项全部定位并进入可视区域、DOM id 全部由 evidenceId 派生、搜索同时作用于
  导航与卡片、请求模式与会话模式组成分段点击定位、1446 行轨迹账本首/中/末行在检查器里定位到
  同一条证据、自动刷新后选中行与检查器分析卡片保持稳定。另扫描 14 条记录（含 HTTP 400 错误记录），
  覆盖模型输出、工具调用、思考内容、用量与未识别事件提示，无一处抛错。
- 浏览器验证中发现并修正一处：轨迹的 `watch(() => props.trajectory)` 原本在每次刷新时无条件清空
  `selectedRowId`，导致自动刷新一开就丢失选中行和检查器。旧行号式行 id（`${record.id}:${kind}:${index}`）
  确实不能跨刷新复用，所以清空是当时的安全做法；改为 evidenceId 派生的行 id 之后可以按 id 判断证据
  是否仍然存在，只有证据真的消失才清空。
- 浏览器验证中发现一处既有问题（本次未改，与本次变更无关）：会话模式组成图在聚合大量请求时，
  分段的最小宽度约束（`0.35%` ≈ 4px）大于其真实占比宽度，相邻小分段会物理重叠，只有最上层分段
  能被指针点到。分段几何计算不在本次改动范围内（`git diff` 仅移动了类型声明位置），需要单独处理。
