...
修正发送控件与消息列表的旧测试所有权，使其从真实父组件 `WebqqChatPane` 检查嵌套关系。定向测试、完整测试、类型检查和构建通过。Ego Browser 任务空间仍由用户接管，未绕过限制；Chromium 全矩阵、Firefox 兼容与最终 DOM 对照保留待复核。验证记录位于 `.scratch/webqq-modularization/evidence/13-page-shell/verification.json`。
