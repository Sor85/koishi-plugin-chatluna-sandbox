...
**Status:** resolved（一项偏离待确认，见 Comments）
...
- [x] 退格跨提及整块删除有断言，含提及位于开头与位于两段文本之间（断言覆盖的是**删除之后的口径**）
- [ ] （偏离，需确认）What to build 里「加上退格」这条接线：退格**没有**改由草稿宿主消费，仍交给提及芯片的原子节点删除。理由与代价见下面「退格：断言落地，接线不动」
