...
- `isComposerMentionBoundary`：光标是不是正贴在一个提及的右边。
- `deleteComposerMentionAtBoundary`：整块删掉那个提及，光标停在它原来的位置。
- `routeComposerKey`：这次按键是什么意思。返回 `none` / `blocked` / `move-candidate` /
  `select-candidate` / `close-menu` / `delete-mention` / `submit` 七种动作之一。
...
「整块删掉提及」这条规则治理前由 `contentEditable = 'false'` 提供——芯片是原子节点，靠的是
各家 contenteditable 对不可编辑节点的处理。本票把它改成由本模块说了算，范围收窄到**一个位置**：

- **光标正贴在芯片右边**（当前文本 token 的偏移为 0，前一个 token 是提及）→ 由宿主整块删掉，
  光标停在提及原来的位置。
- **光标在提及后自动补入的分隔空格之后**（偏移 1）→ 不消费，交给宿主，只删那个空格，芯片留着。
  这是治理前的行为，经确认保留。
- 其余位置的退格一律不消费；组字期间的退格交给输入法。

两个位置正好对应 `deleteComposerBackward` 的两条分支，本票只用与治理前行为一致的那一条
（第 4 条）。**光标落点没有复用它给的**：它把光标放到合并后那个文本 token 的末尾，提及后面
还有正文时那是行尾而不是提及原来的位置，表现为「删掉一个提及，光标却跳到了行尾」。
`deleteComposerMentionAtBoundary` 因此只借它算 token，光标另算——删掉提及之后前后两段文本
合并成一个 token，提及原来的位置就是前一段文本的长度。

### 这次改动的实测代价

按确认后的口径重跑了 23 个采样点（新增「提及位于两段文本之间时退格」那一组四个）。
可见部分全部逐字一致：`editorHtml`、规范化后的整段 DOM、占位文案可见性、`data-empty`、
发送按钮禁用态、控件宽度、候选菜单，两个引擎在 23 个采样点上一处不差。

变化只在 `Selection` 的内部表示上，光标的视觉位置不变：

| 引擎 | 采样点 | 基线 | 本票 | 说明 |
| --- | --- | --- | --- | --- |
| Firefox | 退格两次 | `DIV`, offset 1 | `#text`, offset 3 | 同一个视觉位置的两种坐标写法 |
| Chromium | 两段文本之间的提及退格，及其后 6 个采样点 | 锚点节点文本 `你好  ` | 锚点节点文本 `你好   在吗` | 偏移两边都是 4；基线留下两个相邻文本节点，本票渲染成一个合并节点 |

两处都不是行为变化。第二处顺带把「一个 token 一个节点」这条不变量修得更稳：相邻文本节点一多，
子节点序号与 token 序号就会错位（既有的「换行之后再输入 @ 不弹菜单」正是这条错位的表现），
合并成一个节点少一种触发路径。
...
| 命令 | 结果 |
| --- | --- |
| `yarn test` | 通过。175 个文件、1589 项 |
| `yarn typecheck` | 通过 |
| `yarn build` | 通过。`@vueuse/core` 两处 `#__PURE__` 注解警告是既有的 |
| DOM 快照 ＋ 光标快照（Chromium） | 23 个采样点，可见部分逐字一致；7 处 Selection 锚点节点由「两个相邻文本节点」变成「一个合并节点」，偏移不变 |
| DOM 快照 ＋ 光标快照（Firefox） | 23 个采样点，可见部分逐字一致；1 处 Selection 表示形式由 `DIV, 1` 变成 `#text, 3`，视觉位置不变 |

两个引擎控制台均无错误。证据与脚本见 `../evidence/`。
