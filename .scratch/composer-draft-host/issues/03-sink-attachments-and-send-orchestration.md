...
- [x] 粘贴非图片内容不进附件，有断言（口径：剪贴板里**没有文件**时不进附件也不吞粘贴；粘到一个非图片**文件**时它仍是附件，与选择文件入口一致——治理前如此，见 Comments）
- [x] 文件名与扩展名拆分有断言，含无扩展名与多个点的名字
- [x] 附件的增、删、清空各有断言，含重复加入同一个文件（口径：**允许**重复加入，两条各有标识、可独立移除——治理前如此，见 Comments）
...
- [x] DOM 快照与基线逐像素一致（含贴图后与发送后两个采样点；贴图那一步在 Firefox 里是脚本层面的空操作，见证据 README）
- [x] 完整测试、类型检查与构建通过
...
代码评审（standards 轴）指出媒体载荷的三份同形声明：`ComposerAttachmentMedia`、
`ComposerSendMedia` 与组件里 `WebqqComposerSendIntent.media` 的内联结构。前两份删掉，
两个新模块改用领域类型 `SendMediaFileInput`（`src/types.ts`，也是这条链路末端
`SendMediaMessageInput.media` 的元素类型）。组件里那份内联结构本轮不动——emits 的载荷声明
是本 feature 明确冻结的东西，改它会让「载荷逐字不变」这条断不成立；留作后续候选。
