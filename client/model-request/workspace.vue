    ...
    <div class="webqq-model-request-split">
      <section class="webqq-model-request-list-pane" aria-label="模型请求列表">
        <header class="webqq-model-request-list-toolbar webqq-overlay-header">
          <div class="webqq-model-request-list-heading">
            <h2>请求列表</h2>
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger as-child>
                  <span
                    class="webqq-model-request-capacity"
                  ...
                  >
                    <svg viewBox="0 0 32 32" aria-hidden="true">
                      <circle class="webqq-model-request-capacity-track" cx="16" cy="16" r="13" />
                      <circle class="webqq-model-request-capacity-track" cx="16" cy="16" r="9" />
                      <!-- 两道上限各占一环：外环体积、内环条数。stroke-dasharray 的第一段是已用弧长，
                           周长按 2πr 求出后写死在 dasharray 里，避免每帧再算一次。 -->
                      <circle
                        class="webqq-model-request-capacity-arc is-bytes"
                        cx="16"
                        cy="16"
                        r="13"
                      ...
                      />
                      <circle
                        class="webqq-model-request-capacity-arc is-records"
                        cx="16"
                        cy="16"
                        r="9"
                ...
                <TooltipContent class="webqq-model-request-capacity-tip">
                  <div class="webqq-model-request-capacity-metrics">
                    <span class="webqq-model-request-capacity-swatch is-bytes" aria-hidden="true" />
                    <span>体积</span>
                    <strong :class="{ 'is-near-limit': capacityGauge.byteNearLimit }">{{ capacityGauge.byteUsed }}</strong>
                    <span class="webqq-model-request-capacity-max">/ {{ capacityGauge.byteMax }}</span>
                    <span class="webqq-model-request-capacity-swatch is-records" aria-hidden="true" />
                    <span>条数</span>
                    <strong :class="{ 'is-near-limit': capacityGauge.recordNearLimit }">{{ capacityGauge.recordUsed }}</strong>
                    <span class="webqq-model-request-capacity-max">/ {{ capacityGauge.recordMax }}</span>
                  </div>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      <section class="webqq-model-request-detail-pane" aria-label="模型请求详情">
        ...
        <article v-else ref="detailElement" v-webqq-scrollbar class="webqq-model-request-detail" :class="{ 'has-sticky-trajectory': detailView === 'trajectory' || bodyView === 'analysis', 'is-trajectory-view': detailView === 'trajectory' }">
          <header>
            <div class="webqq-model-request-item-title">
              <span class="webqq-model-request-bot">
                <ModelRequestAvatar
...
function applyEntryState() {
  ...
  emit('trajectory', trajectoryQuery(scope, state.recordId, 'request'))
...
function refresh(limit = MODEL_REQUEST_PAGE_SIZE) {
  ...
  if (selectedRecordId.value) {
    ...
    emit('trajectory', trajectoryQuery(scope, selectedRecordId.value, trajectoryMode.value))
  }
}
...
function openRecord(recordId: string) {
  ...
  emit('trajectory', trajectoryQuery(scope, recordId, trajectoryMode.value))
}
...
function fetchTrajectory(mode: 'request' | 'conversation') {
  const detail = props.detail
  if (!detail) return
  emit('trajectory', trajectoryQuery(resolveRecordScope(detail), detail.id, mode))
}
...
function openRelatedRequest(payload: {
  ...
  const scope = record ? resolveRecordScope(record) : currentScope()
  emit('open', { ...scope, recordId: payload.recordId })
  emit('trajectory', trajectoryQuery(scope, payload.recordId, 'request'))
  nextTick(() => {
...
function returnToTrajectory() {
  ...
  const scope = record ? resolveRecordScope(record) : currentScope()
  emit('open', { ...scope, recordId: state.recordId })
  emit('trajectory', trajectoryQuery(scope, state.recordId, state.trajectoryMode))
  applyViewRestore()
}
