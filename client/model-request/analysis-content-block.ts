...
const AnalysisContentBlock = defineComponent({
  ...
  setup(blockProps, { slots }) {
    ...
    const renderContent = () => {
      const wrapStyle = { '--webqq-model-analysis-collapse-height': collapsedHeight.value }
      if (slots.default) {
        return h('div', {
          ref: textWrap,
          style: wrapStyle,
          class: 'webqq-model-analysis-text-wrap is-payload',
        }, slots.default())
      }
      return h('div', { ref: textWrap, style: wrapStyle, class: 'webqq-model-analysis-text-wrap' }, [
