    ...
    <div class="webqq-model-analysis-main">
      ...
      <div ref="contentElement" class="webqq-model-analysis-content">
        ...
        <div class="webqq-model-analysis-conversation">
          ...
          <header v-if="block.messages.length" class="webqq-model-analysis-heading">
          ...
          >
            ...
            <div v-show="!isCardCollapsed(modelAnalysisTargetId(message.evidenceId)) && !isMessageRaw(message.evidenceId)" class="webqq-model-analysis-formatted">
              ...
              <section v-else-if="messagePayloadTree(message)" class="webqq-model-analysis-section">
                <div class="webqq-model-analysis-tool-payload webqq-model-request-json-viewer">
              ...
              <section v-if="message.toolCalls.length && requestToolCallsVisible" class="webqq-model-analysis-section">
                ...
                >
                  ...
                  <div v-if="callArgumentsTree(call)" class="webqq-model-analysis-tool-payload webqq-model-request-json-viewer">
          ...
          <section v-if="block.variables" class="webqq-model-analysis-variables">
            ...
            <div v-show="!isCardCollapsed(MODEL_ANALYSIS_RESPONSE_TARGET) && !responseRaw" class="webqq-model-analysis-formatted">
              ...
              <section v-if="response.toolResults.length && responseToolResultsVisible" class="webqq-model-analysis-section">
                ...
                >
                  ...
                  <div v-if="toolResultTree(result)" class="webqq-model-analysis-tool-payload webqq-model-request-json-viewer">
          ...
          <section v-if="block.tools" :id="MODEL_ANALYSIS_TOOLS_TARGET" class="webqq-model-analysis-tools" :class="{ 'is-located': highlightedTarget === MODEL_ANALYSIS_TOOLS_TARGET }">
 ...
 * User 之后，卡片里却要翻过整段对话才见到。共用 SANDBOX_EVIDENCE_READING_ORDER 之后两侧
 * 一定同序。
 *
...
const analysisBlocks = computed(() => {
  const grouped = new Map<SandboxEvidenceReadingGroup, ModelConversationMessage[]>()
  for (const message of visibleMessages.value) {
    const key = sandboxEvidenceReadingGroup({ kind: message.kind })
    const bucket = grouped.get(key)
    if (bucket) bucket.push(message)
    else grouped.set(key, [message])
  }
  return SANDBOX_EVIDENCE_READING_ORDER.flatMap((key) => {
    ...
    return [{
      ...
      label: sandboxEvidenceLabels(messages[0]?.kind ?? key).title,
      messages,
      variables,
      response,
      tools,
...
function toggleNavigationGroup(group: SandboxEvidenceReadingGroup) {
...
function groupIcon(group: SandboxEvidenceReadingGroup): Component {
