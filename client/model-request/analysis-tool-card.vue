<template>
  <article
    :id="id"
    class="webqq-model-analysis-card is-tool-call"
    :class="{
  ...
  >
    ...
    >
      <span class="webqq-model-analysis-role">TOOL CALL</span>
      <span class="webqq-model-analysis-tool-card-name">
        <AnalysisHighlightedText :value="title" :query="query" />
      </span>
      <span class="webqq-model-analysis-tool-card-id">
        <AnalysisHighlightedText :value="callId || '无调用 ID'" :query="query" />
      </span>
      <span class="webqq-model-analysis-chars">{{ characters }} chars</span>
      <!-- 定义入口属于卡片操作，不在正文下方另起一行，避免抬高底部的展开按钮。 -->
      <Button
        v-if="showLocateTool"
        variant="ghost"
        size="xs"
        class="webqq-model-analysis-locate-tool"
        aria-label="查看工具定义"
        title="查看工具定义"
    ...
    <div v-show="!isCollapsed" class="webqq-model-analysis-formatted">
