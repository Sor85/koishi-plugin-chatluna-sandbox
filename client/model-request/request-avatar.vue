<template>
  <span
    v-if="unattributed"
    class="webqq-identity-avatar webqq-model-request-unattributed-avatar"
  >
    <!-- 形状取自内置机器人头像（src/builtin-avatar-options.ts 的 bot()）：同一枚机头、
         同一对眼睛、同一条嘴、同一根天线，去掉圆角底板、改描边、收成单色。
  ...
  <WebqqAvatar v-else kind="bot" :name="name" :avatar="avatar" />
</template>
 ...
 * 因此未归属走专属头像——虚线圆里放一枚和真实机器人同形状的机器人，但没有彩色底板、
 * 只有单色描边，再划一道斜线，「有底色的是真机器人，无底无色又被划掉的是找不到的
 * 那一个」这一读法由此成立。
 *
 * 图标是装饰：归属名称就在同一行的标题里，头像再报一次会让读屏念两遍。
 */
defineProps<{
  /** 记录的归属判定结果，不是「有没有 botId」：归属成功但多个机器人同时思考时同样拿不到 botId。 */
