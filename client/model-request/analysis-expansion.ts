...
import type { SandboxEvidenceReadingGroup } from '../../src/evidence-reading-order'
...
export function createAnalysisExpansion() {
  ...
  const collapsedNavigationGroups = ref(new Set<SandboxEvidenceReadingGroup>())
  ...
  function isNavigationGroupCollapsed(group: SandboxEvidenceReadingGroup): boolean {
    return collapsedNavigationGroups.value.has(group)
  }

  /** 左侧导航分组默认全部展开，各自独立折叠；折叠一个分组不改变当前导航目标。 */
  function toggleNavigationGroup(group: SandboxEvidenceReadingGroup): void {
