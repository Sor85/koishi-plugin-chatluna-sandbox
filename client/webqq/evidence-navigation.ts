...
import type { ModelTextRange } from './model-request-occurrence'
import { MAIN_MODEL_REQUEST_SPACE_ID } from './model-request-query'
...
/** 意图的空间归属。模型请求视图只按主环境或某个测试空间进入，不按「全部」或「未归属」。 */
export type EvidenceNavigationScope = Exclude<
  SandboxModelRequestScope,
  { scope: 'all' } | { scope: 'unattributed' }
>
...
/** 一个类型两种形态：必含空间归属与记录标识，可选携带证据定位段。 */
export interface EvidenceNavigationIntent {
  seq: number
  scope: EvidenceNavigationScope
  recordId: string
...
export function createEvidenceNavigation() {
  ...
  /** 从聊天消息进入：单向跳转，不携带证据定位段，也没有回程。 */
  function enterFromMessage(reference: SandboxMessageModelRequestReference): EvidenceNavigationIntent {
    presetOrigin.value = undefined
    presetOriginRestore.value = undefined
    return publish({
      scope: reference.scopeId === MAIN_MODEL_REQUEST_SPACE_ID
        ? { scope: 'main' }
  ...
  function publish(intent: Omit<EvidenceNavigationIntent, 'seq'>): EvidenceNavigationIntent {
    ...
    entryState.value = {
      ...
      category: 'space',
      spaceId: published.scope.scope === 'main'
        ? MAIN_MODEL_REQUEST_SPACE_ID
        : published.scope.spaceId,
      model: '',
      errorsOnly: false,
      detailView: 'evidence',
      bodyView: 'analysis',
