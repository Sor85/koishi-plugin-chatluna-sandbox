<template>
  <main class="chatluna-sandbox-chat webqq-preset-workspace" aria-label="预设工作台">
    ...
    <p v-if="error" class="webqq-preset-error" role="alert">{{ error }}</p>
    ...
    <div class="webqq-preset-layout">
      <aside class="webqq-preset-list-pane" aria-label="预设文件列表">
        ...
        <div v-else ref="listElement" v-webqq-scrollbar class="webqq-preset-groups">
          <section v-for="group in presetGroups" :key="group.kind" class="webqq-preset-group">
            ...
            <p v-if="!group.documents.length" class="webqq-preset-empty">没有匹配的{{ kindLabel(group.kind) }}预设</p>
          </section>
        </div>
      </aside>
      ...
      <section class="webqq-preset-editor-pane" aria-label="预设源码">
        ...
        <template v-else>
          ...
          <footer class="webqq-preset-editor-footer">
    ...
    <Dialog v-model:open="createDialogOpen">
      <DialogContent>
        ...
        <div class="webqq-preset-dialog-fields">
          <label>类型
    ...
    <Dialog v-model:open="renameDialogOpen">
      <DialogContent>
        ...
        <label class="webqq-preset-dialog-fields">新文件名<Input v-model="renameFileName" /></label>
        <DialogFooter>
...
const props = defineProps<{
  catalog: readonly DeepReadonly<SandboxPresetDocument>[]
  document?: DeepReadonly<SandboxPresetDocument>
  loading: boolean
  saving: boolean
  error: string
...
const pendingDocument = ref<DeepReadonly<SandboxPresetDocument>>()
...
const evidenceContext = computed(() => props.evidenceContext)
...
function requestDocumentSwitch(item: DeepReadonly<SandboxPresetDocument>) {
