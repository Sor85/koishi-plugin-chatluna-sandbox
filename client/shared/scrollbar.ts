...
export interface WebQQScrollbarOptions {
  disabled?: boolean
 ...
 *
 * 工作区缩放会从静止指针下方经过并触发 mouseenter/scroll；若此时保留 is-visible，全局动画遮罩
 * 移除后轨道会在最终位置补闪一次，因此缩放期间必须连可见状态也拒绝写入。
 */
function writeScrollbarClasses(state: WebQQScrollbarState) {
  ...
  state.overlay.classList.toggle('is-visible', state.visibility.revealed && state.showOverlay && !workspaceZooming)
...
function updateScrollbar(state: WebQQScrollbarState) {
  ...
  if (accent) overlay.style.setProperty('--webqq-accent', accent)
  ...
  overlay.style.setProperty('--webqq-scrollbar-thumb-top', `${thumbTop}px`)
  overlay.style.setProperty('--webqq-scrollbar-thumb-height', `${state.thumbHeight}px`)
}
...
function applyScrollbarOptions(
  ...
  options: WebQQScrollbarOptions | undefined,
) {
  const { overlay } = state
...
function attach(element: HTMLElement, options: WebQQScrollbarOptions | undefined) {
...
function refresh(element: HTMLElement, options: WebQQScrollbarOptions | undefined) {
...
export interface WebQQScrollbarHandle {
  update(options?: WebQQScrollbarOptions): void
  detach(): void
}
 ...
 * 表头有内容可采样而延伸到表头背后，轨道就会跟着钻进顶栏。
 */
export function attachWebqqScrollbar(
  element: HTMLElement,
  options?: WebQQScrollbarOptions,
): WebQQScrollbarHandle {
