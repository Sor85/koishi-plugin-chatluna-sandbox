...
测试控制端点是一个独立的 HTTP 监听器，在同一个地址和端口上并列提供两种协议表述。两者共用监听地址、TLS、来源与 Origin 白名单、测试凭证与配额，背后是同一个测试控制服务，因此权限、幂等、确认令牌与测试调用记录的语义完全一致。插件全局配置里「测试控制端点」分组下有三个子分组：`MCP 测试端点` 与 `HTTP 测试端点` 各有一个总开关（默认都开启，可以按需单独关掉其中一种）和自己的路径，`端点通用设置` 放来源与 Origin 白名单、TLS 与全部调用配额。
