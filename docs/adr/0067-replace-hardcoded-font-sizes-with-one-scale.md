...
标度定义必须列出全部渲染根：`.chatluna-sandbox-page`，以及 Dialog / Popover / Select / Tooltip / 右键菜单 / 浮动二级页 / 合并转发弹层 / 图片预览这些 teleport 到 `body` 的浮层。它们不在工作区的继承树里，漏掉任何一个，那棵子树里的 `var(--webqq-font-*)` 会解析失败并静默退回继承字号——失败形态和改造前一模一样。清单存放在 `scripts/sandbox-style-roots.mjs`（同一份清单还决定 Tailwind preflight 的作用域，见 ADR 0068），`tests/sandbox-style-roots.test.ts` 断言 CSS 里的选择器列表与它逐项一致。这条纪律是补上的：本决策初版只列了八个根，漏掉了 `dialog-overlay`、`tooltip-content`、合并转发弹层与图片预览，合并转发弹层里的 `--webqq-font-lg` 与 `--webqq-font-md` 因此解析失败并退回 16px。

shadcn 组件保留自己的 `text-sm`（14px）默认值，不做覆盖。它们是标准控件，覆盖尺寸会破坏 registry 组件 1:1 还原；14px 也已经在标度上（`lg`），只是密度比本插件的 13px 正文略松。

`tailwind.generated.css` 的 preflight 曾把 `line-height: 1.5` 和 `font-family` 写在未作用域限定的 `html` 选择器上，这是本插件对整个控制台的全局副作用。工作区因此不再依赖它：`.webqq-workspace` 自己写死 `line-height: 1.5`。preflight 的作用域问题本身由 ADR 0068 处理。
