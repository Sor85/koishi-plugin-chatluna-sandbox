...
WebQQ 工作台采用纯结构性重构，保持现有 DOM、CSS class、视觉、动画、响应式断点和领域行为不变。Vue 视图按左侧栏、聊天区域、发送控件、右侧信息栏和跨区域 Dialog 覆盖层拆分，逻辑则按工作区状态、布局状态、覆盖层状态、参与者切换和消息交互拆分；`page.vue` 最终只负责工作台装配。
