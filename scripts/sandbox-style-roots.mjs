...
//
// 两处消费这份清单：
//   1. client/styles/webqq-tokens.css 在这些选择器上定义排版标度令牌；
//   2. scripts/scope-preflight.mjs 把 Tailwind preflight 限制在这些子树内。
// 两者的失效形态都是"静默退回浏览器默认值"，不会报错，所以
// tests/sandbox-style-roots.test.ts 断言模板里每个 Portal 根都在这份清单里，
// 且 webqq-tokens.css 的令牌规则选择器列表与本清单逐项一致。
//
// 新增 teleport / Portal 浮层时必须同时补进这里和 webqq-tokens.css。
export const SANDBOX_STYLE_ROOTS = [
  // Koishi k-layout 的 main 容器，覆盖 .webqq-workspace 及其全部后代。
  ...
  // 自建浮层：浮动二级页（贴表情、查看资料）、合并转发弹层、图片预览。
  '.chatluna-sandbox-secondary-page',
  '.chatluna-sandbox-forward-modal-backdrop',
  '.chatluna-sandbox-image-preview',
]
