...
雾化二级面使用半透明背景，并通过 `backdrop-filter` 模糊其后方内容。典型场景包括启用毛玻璃外观（`enableSandboxFrostedGlass`）的查看资料、贴表情和通知面板。
...
- 支持外观切换的浮动二级页（查看资料、贴表情）同时挂 `.chatluna-sandbox-solid-secondary-surface` 与动态 `.is-frosted`：开关开启时回落基类雾化样式，关闭时由实体规则覆盖为不透明背景。实体覆盖必须同时覆写背景色，只关闭 `backdrop-filter` 会穿帮成"半透明但不模糊"的中间态。
